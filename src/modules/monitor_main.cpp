...
void MonitorMain::Start()
{
    Init();
    {
        std::thread thread_(std::bind(&MonitorMain::Monitor, this));
        thread_.detach();
    }
    {
        std::thread thread_(std::bind(&MonitorMain::Saver, this));
        thread_.detach();
    }
    {
...
void MonitorMain::Init() 
{
    ...
    auto monitorItems = config_.GetValueJson("monitorItems");
    ...
    csv_ = cuCsv_.CreateCsv(columnNames);
...
void MonitorMain::Monitor()
{
    ...
    for (;;) {
        ...
        csv_ = cuCsv_.AppendCsvRow(csv_, data);
        usleep(reflashIntervalMs_ * 1000);
    }
}
...
void MonitorMain::Saver()
{
    ...
    for (;;) {
        auto saveCsv = csv_;
        WriteFile(outputPath_, cuCsv_.PrintCsv(saveCsv, ","));
...
{
    topAppPkgName_ = GetPtrData<std::string>(data);
}
